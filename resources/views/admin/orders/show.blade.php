<x-admin-layout>
    <div class="mb-6 flex flex-wrap items-center justify-between gap-4">
        ...
        <a class="rounded-full border border-sand-200 px-3 py-1.5 text-sm font-medium text-ink-700 transition hover:border-sand-300 hover:text-ink-900 ui-ring" href="{{ route('admin.orders.index') }}">Back</a>
    </div>
    ...
    @if ($errors->has('status'))
        <x-ui.alert class="mb-4" variant="danger" title="Order update issue">
    ...
    <form method="POST" action="{{ route('admin.orders.update', $order) }}" class="mb-6 flex flex-wrap items-center gap-3" x-data="{ submitting: false }" x-on:submit="submitting = true">
        @csrf
        @method('PUT')
        <x-select-input name="status" class="min-w-[180px]" aria-label="Order status">
            @foreach ($statuses as $status)
                <option value="{{ $status->value }}" @selected($order->status === $status)>{{ $status->label() }}</option>
            @endforeach
        </x-select-input>
        <x-secondary-button x-bind:disabled="submitting">
            <span x-show="!submitting">Update status</span>
            <span x-show="submitting" x-cloak>Updating...</span>
        </x-secondary-button>
        <x-ui.loading x-show="submitting" x-cloak label="Updating order..." />
    </form>

    @if ($order->items->isEmpty())
        <x-ui.empty-state title="No order items" description="This order does not have line items yet." />
    @else
        <x-ui.table>
